...
def check_exists(f):
    ...
    '''
    r = RedisOp()
    tid = '%s_%s' % (f.get('kid'), f.get('id'))
    print tid
    if r.check_set_exists('backtrack_store_set', tid):
        pass
    else:
        r.push_set_data('backtrack_store_set', tid)
        return f 
...
def send_messages(kid, keyword, feeds):
    ...
    db = conn['backtracking']
    for feed in feeds:
        db[str(kid)].insert(feed)
    conn.close()
